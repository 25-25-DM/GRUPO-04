...
class VehiculoViewModel(private val repository: AppRepository) : ViewModel() {
    ...
    private val _usuarioActual = MutableStateFlow<String>("")
    val usuarioActual = _usuarioActual.asStateFlow()

    fun establecerUsuarioActual(nombre: String) {
        _usuarioActual.value = nombre
    }

    // Estado para la cadena del último login (mensaje que se muestra en la UI)
    private val _ultimoLogin = MutableStateFlow("Bienvenido")
    val ultimoLogin = _ultimoLogin.asStateFlow()
    ...
    fun cargarUltimoLogin(nombreUsuario: String) {
        viewModelScope.launch {
            repository.obtenerUltimoLoginUsuario(nombreUsuario)
                .collect { mensaje ->
                    _ultimoLogin.value = mensaje
                }
        }
    }
