...
class AppRepository(
    ...
    suspend fun actualizarVehiculo(vehiculo: Vehiculo, context: Context): Result<Unit> = withContext(Dispatchers.IO) {
        // 1. Verifica si el vehículo está inactivo
        if (!vehiculo.activo) {
            try {
                // 2. Llama al servicio web msgedit para obtener el mensaje personalizado
                val mensaje = obtenerMensajeEdicionInvalida()
                mostrarNotificacionLocal(
                    context,
                    "Edición bloqueada",
                    mensaje
                ...
                val mensaje = "Imposible editar el vehículo"
                mostrarNotificacionLocal(
                    context,
                    "Edición bloqueada",
                    mensaje
    ...
    private fun sincronizarItemHaciaNube(
        logTag: String,
        tableName: String,
    ...
    fun obtenerUltimoLoginUsuario(nombre: String): Flow<String> = flow {
        val usuario = usuarioDao.getUsuarioPorNombre(nombre)
        emit(
            if (usuario?.lastLogin != null)
                "Último acceso: ${usuario.lastLogin}"
            else
                "Bienvenido"
        )
    }
    suspend fun actualizarLastLogin(nombre: String, fecha: String) {
        usuarioDao.actualizarLastLogin(nombre, fecha)
    }
...
fun obtenerUltimoLoginUsuario(usuarioDao: UsuarioDao, nombre: String): Flow<String> = flow {
    val usuario = usuarioDao.getUsuarioPorNombre(nombre)
    emit(
        if (usuario?.lastLogin != null)
            "Último acceso: ${usuario.lastLogin}"
...
private suspend fun obtenerMensajeEdicionInvalida(): String = withContext(Dispatchers.IO) {
    val url = URL("https://tu-api.com/msgedit")
    val connection = url.openConnection() as HttpURLConnection
    ...
    return@withContext try {
        ...
        if (connection.responseCode == 200) {
            ...
            throw IOException("Error al obtener mensaje desde msgedit. Código: ${connection.responseCode}")
        }
    } finally {
